...
Socket* Listener::acceptConnection(){
	 ...
	 * create different memory address for each accepted socket descriptor.
	 * this prevent the previous socket descriptor points to the recently socket descriptor due to local variable.
	 */
	int* newSocket = new int(accept(socketDescriptor, (struct sockaddr*) &socketAddress, &addrlen));
	/*
	 * dynamic memory allocaiton.
	 * create different memory address fo each socket.
	 * this prevent the the previous created socket point to the recent created socket due to local variable.
	 */
	Socket* socket = new Socket(*newSocket);
	return socket;
}
